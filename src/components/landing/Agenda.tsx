    ...
    <section id="agenda" className="py-20 px-6 bg-white">
      <div className="max-w-6xl mx-auto">
          ...
          {agenda.map((day, index) => (
            <button
              ...
              className={cn(
                ...
                activeDay === index
                  ? "text-[#0B4F6C] border-b-2 border-[#0B4F6C]"
                  : "text-gray-500 hover:text-[#0B4F6C]",
              )}
            >
              {day.date}
            </button>
        ...
        <div className="relative">
          {/* Timeline line */}
          <div className="absolute left-0 md:left-1/4 top-0 bottom-0 w-px bg-gray-300 ml-6 md:ml-0"></div>
              ...
              <div key={session.id} className="relative">
                <div className="flex flex-col md:flex-row">
                  ...
                  <div className="md:w-1/4 mb-4 md:mb-0 flex">
                    <div className="relative z-10 w-12 h-12 rounded-full bg-[#01BAEF] flex items-center justify-center text-white font-bold ml-0 md:-ml-6">
                      {session.id}
                    </div>
                    <div className="ml-4 md:ml-6">
                      <p className="font-bold text-[#0B4F6C] font-['Roboto']">
                        {session.time}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="md:w-3/4 pl-12 md:pl-0">
                    <div
                      className={cn(
                        "bg-white border border-gray-200 rounded-lg p-6 shadow-sm hover:shadow-md transition-all cursor-pointer",
                    ...
                    >
                      ...
                      {session.speaker && (
                        <p className="text-[#01BAEF] font-medium mb-2 font-['Inter']">
                          {session.speaker}
                        </p>
                      )}
                      ...
                      <div className="mt-2 text-sm text-[#20BF55] font-medium">
                        {expandedSession === session.id
                          ? "Click to collapse"
                          : "Click for details"}
                      </div>
